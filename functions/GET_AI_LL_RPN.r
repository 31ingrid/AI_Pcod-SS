...
GET_AI_LL_RPN<-function(species=srv_sp_str,FYR=LLsrv_start_yr){
     ...
     AIrpn <- sqlQuery(CHINA, query = paste0("
			  ...
			  council_sablefish_management_area in ('Aleutians')
                order by  year asc
                ")) %>% 
  rename_all(tolower)
...
GET_AI_LL_LENGTH<-function(species=srv_sp_str,FYR=LLsrv_start_yr){
     ...
     lens <- sqlQuery(CHINA, query = paste0("
                ...
                where     species_code =",species," and 
                          country = 'United States' and 
                          year >=",FYR," and
                          council_sablefish_management_area in ('Aleutians') and
                          length < 999
                order by  year asc
                ")) %>% 
      rename_all(tolower)
