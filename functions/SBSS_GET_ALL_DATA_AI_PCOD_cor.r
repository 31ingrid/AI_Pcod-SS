...
## ----- Get survey shelf biomass estimates -----
  ...
  AI_BIOM <- GET_AI_BIOM(srv_sp_str)
  AI_BIOM$index <- 4
...
## pulling in LL survey RPN data from Dana's csv files.
 LL_RPN<-GET_AI_LL_RPN(species=srv_sp_str,FYR=LLsrv_start_yr )
...
gridc<-expand.grid(year=min(LL_CPUE$year):max(LL_CPUE$year))
  LL_CPUE<-merge(LL_CPUE,gridc,by="year",all=T)
  LL_CPUE$seas<- 7
  LL_CPUE$index<- 5
  LL_CPUE[is.na(LL_CPUE)]<- 1
  LL_CPUE$index[LL_CPUE$obs==1]<- -5
...
## AI bottom trawl survey size comp
  SRV_LCOMP_SS <- data.frame(GET_AI_LCOMP1(species=srv_sp_str,bins=len_bins,bin=TRUE,SS=TRUE,seas=7,flt=4,gender=0,part=0,Nsamp=100))
...
## ----- Get Fishery catch at size composition data -----
 ...
 FISHLCOMP<-data.frame(GET_AI_LENCOMP2(fsh_sp_str1=202, len_bins1=len_bins, fsh_start_yr1=fsh_start_yr, new_SS_dat_year1= new_year, seas=1,gender=0,part=0,Nsamp=-1)) 
 names(FISHLCOMP) <- c("Year","Seas","FltSrv","Gender","Part","Nsamp",len_bins)



if(AUXFCOMP > 0){
 auxFLCOMP<-LENGTH_BY_CATCH_AI(fsh_sp_str=fsh_sp_str ,fsh_sp_label = fsh_sp_label,ly=new_year)
...
auxflCOMP1=data.frame(Year=auxFLCOMP$YEAR,Seas=rep(1, nrow(auxFLCOMP)),FltSrv=auxFLCOMP$FltSrv,gender=rep(0, nrow(auxFLCOMP)),Part=rep(0, nrow(auxFLCOMP)),Nsamp=auxFLCOMP$Nsamp,auxFLCOMP[,3:(ncol(auxFLCOMP)-2)])
...
FISHLCOMP<-subset(FISHLCOMP,!is.na(FISHLCOMP$Nsamp))
...
## pulling in longline survey length composition data from Dana's csv files 
  ...
  LL_length<-GET_AI_LL_LENGTH(species=srv_sp_str,FYR=LLsrv_start_yr)
  ...
  LL_LENGTH<-data.frame(Year=SS3_LLL$year,Seas=7,FltSrv=5,Gender=0,part=0,Nsamp=100)
...
## get age comp
      AI_ACOMP<-GET_AI_ACOMP1(srv_sp_str=srv_sp_str, max_age=max_age,Seas=7,FLT=-4,Gender=0,Part=0,Ageerr=1,Lgin_lo=-1,Lgin_hi=-1,Nsamp=100)
      print("Survey agecomp done")

      AI_ACOMPF<-LENGTH4AGE_BY_CATCH_AI(fsh_sp_str=202 ,fsh_sp_label = "'PCOD'",sy=fsh_age_start_yr,ly=new_year, STATE=1, max_age=max_age)

      ## Note that these aren't used in the current model so are turned off here...
      AI_ACOMPF$FltSrv<-AI_ACOMPF$FltSrv*-1 

      names(AI_ACOMPF)<-names(AI_ACOMP)
      print("Fisheries agecomp done")
      

      cond_age_length<-data.frame(cond_length_age_cor(max_age1=max_age)$norm)
      names(cond_age_length)<-names(AI_ACOMP)
...
## negating the older fish ages from the file
      ...
      names(cond_age_lengthFISH)<-names(AI_ACOMP)
      print("Conditional fisheries age length done")     
      
      ACOMP<-rbind(AI_ACOMPF,AI_ACOMP,cond_age_lengthFISH,cond_age_length)
      ...
      Age<-GET_SURV_AGE_cor(sp_area=fsh_sp_area,srv_sp_str=srv_sp_str,start_yr=srv_start_yr,max_age=max_age)
