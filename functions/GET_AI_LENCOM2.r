GET_AI_LENCOMP2<-function(fsh_sp_str1=202, len_bins1=len_bins, fsh_start_yr1=fsh_start_yr, new_SS_dat_year1= new_SS_dat_year, seas=1,gender=1,part=0,Nsamp=-1) {
   require(data.table)
  ...
  test <- paste("SELECT CASE  \n", 
      ...
      "WHERE NORPAC.FOREIGN_HAUL.GENERIC_AREA between 540 and 543 \n ",
...
## proportion lengths for each gear type based on haul size (extrapolated number of fish in haul) 
 ...
 test <- paste("SELECT \n ",
      ...
      "WHERE OBSINT.DEBRIEFED_HAUL.NMFS_AREA BETWEEN 540 AND 543 \n",
